...
formalised as a predicate on an object in a category expressing that all
...
\verb|A| to
an object \verb|B|, and a point to \verb|B|, contains a point to \verb|A| and a
proof that the point to \verb|A|
...
$\phi$ to expand the $\phi$ $\circ$ \verb|u| within the definition of
...
composition to change this to \verb|g'|. This is done using
\AgdaFunction{∘-resp-l} which allows a proof to be applied to change the lefthand
...
\\
Before being able to manipulate this expression the expression must be
...
specific instances, Cantor's theorem and the first fixed point theorem
...
to the author as to whether point-surjectivity has a more coherent interpretation
...
syntactic applicative structure adds a method of interpreting terms in the
$\lambda$-calculus, elements of the set $\Lambda$, into the applicative
structure. In other words a syntactic applicative structure is a triple $M = (A
\, , \, \bullet \, , \, \llbracket \, \rrbracket)$ of an underlying set $A$, a
binary operation $\bullet : A \rightarrow A$ and a syntactical interpretation
...
under their interpretation within the $\lambda$-algebra. There is a close
...
Interpreting within the context of the transformation from \textit{Karoubi
...
\textbf{Definition 5.5.3.} defines a semantic interpretation function for
...
theorem under application. Whilst not the briefest or most insightful proof, it 
 raises further questions concerning categorical interpretations
...
which other equational theories hold of their interpretation of $\lambda$-terms.
...
interpretation function giving rise to $\lambda$-algebras. This thesis does not
