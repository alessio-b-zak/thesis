...
not incidental; many of the theorems abstracted by the theorem play an
...
\begin{thm}
    fet $f$ be a function from a set $A$ to its powerset $\mathcal{P}(A)$. Then
    $f$
    is not a surjective function.
\end{thm}

\begin{proof}
    fiming for a contradiction assume $f$ is surjective. Consider the set $B =
...
na{\"i}vity of Cantor's foundations rested in the Axiom of Unrestricted
Comprehension, orignally taken from work by Gottlob Frege
\cite{frege1884grundlagen}

\begin{definition}
\end{definition}

This lead to perhaps the first signifcant result in the foundations of
mathematics in the 20th century, Russell's paradox. Betrand Russell
(1872-1970), an English polymath, showed in \textit{The Principles of
Mathematics} \cite{russell1903principles} written with Alfred Whitehead in 1903
...
and Alan Turing. \todo{make better}Turing's formalisation
